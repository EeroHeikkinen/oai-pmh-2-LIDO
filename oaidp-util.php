...
{
	global $SQL;
	$query = "SELECT * FROM ".$SQL['table'];
	$where = array();
	if (!empty($SQL['metadataPrefix'])) 
		$where[] = $SQL['metadataPrefix']." LIKE '%$metadPrefix%'";
	if (!empty($id)) {
	...
	switch(count($where)) {
		...
		case 2:
			return $query . " WHERE " . $where[0] . " AND " . $where[1];
	}
	return $query;
}
...
function rowCount($metadataPrefix, $extQuery, $db) {
	global $SQL;
	$n = 0;
	$sql = "SELECT COUNT(*) FROM ".$SQL['table'] . " WHERE ". (!empty($SQL['metadataPrefix'])? ($SQL['metadataPrefix']." LIKE '%$metadataPrefix%'") : '1') . $extQuery;
